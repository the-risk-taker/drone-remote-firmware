...
uint8_t ProtocolBytesProcessing::begin(void)
{
	uint8_t status = 0;
	...
	{
		status = LongRangeRadio.init(NRF24L01_DRONE_REMOTE_RADIO_IDENTIFIER, NRF24L01_CE_PIN, NRF24L01_CSN_PIN);
	}
	...
	if(_interfaceType == SERIAL_INTERFACE)
	{
		while (this->_UARTObject.available() > 0)
		{
			...
			else
			{
				this->_UARTObject.read();
				readStatus = false;
				Serial.println(String(millis()) + "\tNot good, data was lost. Make PROTOCOL_MAX_RX_BUFFER_LENGTH more bigger, or process bytes more frequently.");
			}
		}
...
bool ProtocolBytesProcessing::writeBytes()
{
	...
	else if(_interfaceType == NATIVE_INTERFACE)
	{
		...
		// In this case it is NRF24L01_DRONE_BASE_RADIO_IDENTIFIER;
		writeStatus = LongRangeRadio.send(NRF24L01_DRONE_BASE_RADIO_IDENTIFIER, &this->txBuffer, this->txBufferCursor, NRFLite::NO_ACK);
		
	#if(PROTOCOL_BYTES_OUTPUT_DATA_SNIFF == 0x01)
		this->printBuffer(this->txBuffer, this->txBufferCursor);
	#endif
	}
