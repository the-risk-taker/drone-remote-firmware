		...
		switch(processByteMachine)
		{
			case PREAMBLE:
			{
				...
				switch(preambleMachine)
				{
					...
					default:
					{
						Serial.println(String(millis()) + "\tSomething goes very bad. Unknown state in preamble processing!");
						preambleMachine = FIRST_SIGN;
				...
				switch(crcMachine)
				{
					...
					case CRC_MSB:
					{
						...
						if(processedPacket.crc == processedPacketCRC)
						{
							...
							{
								Serial.println(String(millis()) + "\tPacket was overwritten! Make PROTOCOL_PACKETS_MAX_NUMBER_OF_PACKETS bigger.");
							}
						...
						else
						{
							...
							Serial.println(String(millis()) + "\tSomethink goes very bad. CRC is incorrect! Packet is corrupted!");
						}
					}
					break;
					default:
					{
						Serial.println(String(millis()) + "\tSomethink goes very bad. Unknown process CRC state!");
					}
				}
			}
			break;
			default:
			{
				Serial.println(String(millis()) + "\tSomethink goes very bad. Unknown processBytes() state!");
			}
		}
...
void ProtocolPacketsProcessing::printPacket(sProtocolPacket packet)
{
	...
	for(uint8_t i = 0; i < packet.length; i++)
	{
		...
		if(i < packet.length - 1){ Serial.print(" | "); }
	}
	if(packet.length == 0){ Serial.print("-"); }
...
void ProtocolPacketsProcessing::copyRxBufferToCircualBuffer()
{
	...
	if(tempCounter)
	{
		for(uint8_t i = 0; i < tempCounter; i++)
		{
			...
			{
				Serial.println(String(millis()) + "\tNot good, data was overwritten! Make protocol processing frequency bigger or make PROTOCOL_PACKETS_MAX_CIRCULAR_BUFFER_LENGTH bigger.");
			}
		}
	}
}
...
void ProtocolPacketsProcessing::rewritePacketToBytes(sProtocolPacket &outcomingPacket)
{
	...
	if(tempTxBuffer > PROTOCOL_BYTES_MAX_TX_BUFFER_LENGTH)
	{
		...
		Serial.println(String(millis()) + "\tNot good, buffer too small! Make PROTOCOL_BYTES_MAX_TX_BUFFER_LENGTH bigger.");
	}
	else
	{
